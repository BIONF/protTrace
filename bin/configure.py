...
class setParams:
    ...
    def string_option(self, line_name_value_pair, name):
        ...
        try:
            return str(line_name_value_pair[name])
        except KeyError:
            raise KeyError('The key {0} could not be found in the '
                           'configuration file'.format(name))
    ...
    def read_config_file(self, config_file):
        ...
        with open(config_file, 'r') as config:
            for line in config:
                if not line == '' and line[0] is not None:
    ...
    def parse_key_value_pairs(self, config_file):
        ...
        for line in self.read_config_file(config_file):
            ...
            key_value_pairs.add({splitted_line[0]: splitted_line[1]})
    ...
    def __init__(self, config_file):
        ...
        # Yes-No settings
        self.search_oma_database = self.boolean_option(configs['search_oma_database'])
        self.orthologs_prediction = self.boolean_option(configs['orthologs_prediction'])
        self.run_hamstr = self.boolean_option(configs['run_hamstr'])
        self.run_hamstrOneSeq = self.boolean_option(configs['run_hamstrOneSeq'])
        ...
        self.traceability_calculation = self.boolean_option(configs['traceability_calculation'])
        self.calculate_scaling_factor = self.boolean_option(configs['calculate_scaling_factor'])
        self.calculate_indel = self.boolean_option(configs['calculate_indel'])
        self.perform_msa = self.boolean_option(configs['perform_msa'])
        self.delete_temp = self.boolean_option(configs['delete_temporary_files'])
        self.reuse_cache = self.boolean_option(configs['reuse_cache'])
        self.mapTraceabilitySpeciesTree = self.boolean_option(configs['map_traceability_tree'])
        self.includeParalogs = self.boolean_option(configs['include_paralogs'])
        self.phylogeneticTreeReconstruction = self.boolean_option(configs['orthologs_tree_reconstruction'])
        self.run_spartaABC = self.boolean_option(configs['run_spartaABC'])
        self.evolve_dawg = self.boolean_option(configs['dawg_instead_of_indelible'])

        # Options where numbers or strings can be specified
        self.aa_substitution_matrix = self.string_option(configs['aa_substitution_matrix'])
        self.simulation_runs = self.integer_option(configs['simulation_runs'])
        self.nr_processors = self.integer_option(configs['nr_of_processors'])

        # Default evolution simulation parameters
        self.default_indel = self.float_option(configs['default_indel'])
        self.default_indel_distribution = self.float_option(configs['default_indel_distribution'])
        self.default_scaling_factor = self.float_option(configs['default_scaling_factor'])
        ...
        self.hamstr_environment = self.string_option(configs['hamstr_environment'])
        ...
        self.path_work_dir = self.create_path_option(configs['path_output_dir'])
        ...
        self.path_distance_work_dir = self.create_path_option(configs['path_distances'])

        # Input file paths
        self.species_MaxLikMatrix = self.path_option(configs['species_MaxLikMatrix'])
        self.hamstr_oma_tree_map = self.path_option(configs['Xref_mapping_file'])
        self.species_MaxLikMatrix = self.path_option(configs['species_MaxLikMatrix'])
        ...
        self.species_tree = self.path_option(configs['reference_species_tree'])
        self.simulation_tree = self.path_option(configs['simulation_tree'])
        self.concat_alignments_script = self.path_option(configs['concat_alignments'])
