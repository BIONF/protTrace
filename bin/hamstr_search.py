...
def actual_hamstr_computation(dirs):
	if not ".fa" in dirs and not ".sql" in dirs:
		...
		if hamstr_name in hamstr_species_in_tree:
			...
			if include_paralogs:
				command = '%s/bin/hamstr.pl -central -sequence_file=%s -taxon=misc -hmmset=%s -strict -checkCoorthologsRef -outpath=%s -hit_limit=10 -blastpath=%s' %(hamstr, file_name, protein_id, output, blastDir)
	...
	if count > 0:
		...
		for i in range(len(s1)-1):
			if '>' in s1[i]:
				...
				if not target_species_id in list(speciesList.keys()):
		...
		for files in glob.glob(output+'/*.out'):
			...
			for j in range(len(s3)-1):
				if s3[j].split()[0] == hamstr_name:
					...
					break
			#print oma_name[:5]
			if not oma_name in list(speciesList.keys()) and hamstr_name in present_species:
				...
				for i in range(len(s2) - 1):
					...
					if not targetSequence in speciesList[oma_name]:
						...
						c += 1
			elif oma_name in list(speciesList.keys()) and hamstr_name in present_species:
...
def finalEditOrthologsSeqs():
	...
	for i in range(0, len(s1) - 2, 2):
		if s1[i][0] == '>'and s1[i][3:8] in species_in_tree:
			fnew.write(s1[i] + '\n' + s1[i+1] + '\n')
	fnew.close()
...
def main(hamstrFile, coreOrtholog, protId, hamstrMapOma, formatdb, blast, del_temp, hamstr_environment, include_para, nr_processors):
